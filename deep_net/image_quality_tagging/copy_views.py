...
def get_uploaded_image(request):
    http_response = render(
        request, '../templates/image_quality_tagging/index.html')
...
def get_predicted_tag(request):
    ...
    if request.method == 'POST':
        ...
        newdoc = Document(docfile=request.FILES['img_upload_button'])
        newdoc.save()

        file_name = str(request.FILES['img_upload_button'].name)
        file_name = file_name.replace(" ", "_")
        file_path = settings.MEDIA_ROOT + "/documents/" + file_name
        img_actual = Image.open(file_path)
        img_actual = img_actual.convert("L")
        img_actual = img_actual.resize((50, 50))
        list(img_actual.getdata())
        ...
        #     return HttpResponseRedirect(reverse('deep_net.views.upload_image'))
    else:
